<x-admin-card>
    ...
    <div class="w-100 justify-content-center">
        <form action="/dashboard/companies/{{$company->hex}}/rankings/update" method="POST" class="w-50">
            @csrf
            @method('PUT')
            ...
            <input 
                ...
                value="{{old('turnover') ?? $ranking->turnover}}"
            ...
            <input 
                ...
                value="{{old('employees') ?? $ranking->employees}}"
            ...
            <input 
                ...
                value="{{old('training_rate') ?? $ranking->training_rate}}"
            >
            @error('training_rate')
                <p class="text-danger">{{$message}}</p>
            @enderror
            ...
            <select name="confirmed_by_company" class="form-select mb-3">
                ...
                <option value="1" {{($ranking->confirmed_by_company == true) ? 'selected' : null}}>Yes</option>
                <option value="" {{($ranking->confirmed_by_company == false) ? 'selected' : null}}>No</option>
            </select>
